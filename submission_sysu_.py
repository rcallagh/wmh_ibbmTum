...
from keras.layers import Input, merge, MaxPooling2D, UpSampling2D, Cropping2D, ZeroPadding2D, Conv2D, Concatenate
from keras.optimizers import Adam
from evaluation import getDSC, getHausdorff, getLesionDetection, getAVD, \
    getImages  # please download evaluation.py from the WMH website
...
def Utrecht_preprocessing(FLAIR_image, T1_image):
    ...
    T1_image = T1_image[:, (image_rows_Dataset // 2 - rows_standard // 2):(image_rows_Dataset // 2 + rows_standard // 2),
               (image_cols_Dataset // 2 - cols_standard // 2):(image_cols_Dataset // 2 + cols_standard // 2)]
...
def Utrecht_postprocessing(FLAIR_array, pred):
    ...
    original_pred[:, (image_rows_Dataset - rows_standard) // 2:(image_rows_Dataset + rows_standard) // 2,
    (image_cols_Dataset - cols_standard) // 2:(image_cols_Dataset + cols_standard) // 2] = pred[:, :, :, 0]
...
def GE3T_preprocessing(FLAIR_image, T1_image):
    ...
    (cols_standard / 2 - image_cols_Dataset / 2):(cols_standard / 2 + image_cols_Dataset / 2)] = FLAIR_image[:,
                                                                                                 start_cut:start_cut + rows_standard,
                                                                                                 :]
    ...
    T1_image_suitable[:, :,
    (cols_standard - image_cols_Dataset) / 2:(cols_standard + image_cols_Dataset) / 2] = T1_image[:,
                                                                                         start_cut:start_cut + rows_standard,
                                                                                         :]
    # ---------------------------------------------------
...
def GE3T_postprocessing(FLAIR_array, pred):
    ...
    image_cols_Dataset = np.shape(FLAIR_array)[2]
    original_pred = np.ndarray(np.shape(FLAIR_array), dtype=np.float32)
    original_pred[:, start_cut:start_cut + rows_standard, :] = pred[:, :, (rows_standard - image_cols_Dataset) / 2:(
                                                                                                                               rows_standard + image_cols_Dataset) / 2,
                                                               0]
...
imgs_three_datasets_two_channels = np.load('wmh_dataset_preprocessed/images_three_datasets_sorted.npy')
imgs_mask_three_datasets_two_channels = np.load('wmh_dataset_preprocessed/masks_three_datasets_sorted.npy')
slices_patient_id_label = np.load('wmh_dataset_preprocessed/slices_patient_id_label.npy')
for dir_name in dirs:
    print('dir_name is:')
    print(dir_name)
    if patient_count < 50:
        inputDir = input_dir_1
    elif patient_count > 49 and patient_count < 70:
        inputDir = input_dir_2
    elif patient_count > 99:
    ...
    for iiii in range(ensemble_parameter):
        model = get_unet(img_shape)
        model_checkpoint = ModelCheckpoint('weights_three_datasets_two_channels_LOOV_' + str(iiii) + '.h5',
    ...
    for iii in range(np.shape(imgs_test_selected)[0]):
        ...
        pred_mask = np.zeros([imgs_mask_test_selected[iii].shape[0], imgs_mask_test_selected[iii].shape[1], 3],
                             dtype=np.float32)
        ...
        matplotlib.imshow(imgs_test_selected[iii, :, :, 0], seg_img[:, :, 0], pred_mask, pred[iii, :, :, 0],
               title=['test image', 'ground truth', 'prediction', 'heat map'])
